...
class BDD100k_dataset(Dataset):
    ...
    def _get_json_targets(self, scale, json_file_name="targets.json"):
        
        x_scale, y_scale = scale[1], scale[0]
        #x_scale, y_scale = 1, 1

        # Open the json file
        with open(self.root_directory + json_file_name, 'r') as f:
            #targets = json.load(f)
            json_data = f.read()
        ...
        for target in targets:
            ...
            if not target["name"].split('.')[0] in self.all_images_name:
            ...
            self.images_name.append(target["name"].split('.')[0])

            if len(self.images_name) >= self.max_size:
                break
    ...
    def __init__(self, dataset_cfg, root_directory, max_size, classes_as_known, classes_as_unknown, combine=False, transform=None):
        ...
        self.all_images_name = []
        for i, image_path in enumerate(self.images_path):

            image_name = image_path.split("/")[-1].split(".")[0]
            self.all_images_name.append(image_name)

        # Load Datasets targets
        self.labels, self.boxes, self.knowns, self.unknowns, self.images_name = [], [], [], [], []
        self._get_json_targets((resize[0]/image_size[0], resize[1]/image_size[1]))
        if self.semantics :
            self._get_json_targets((resize[0]/image_size[0], resize[1]/image_size[1]), json_file_name="ss_targets.json")
    ...
    def semantic_segmentation_to_binary_objects(self, semantic_segmentation):

        semantic_segmentation_binary_objects = semantic_segmentation.detach().clone()
        ...
        semantic_segmentation_binary_objects[mask_objects] = 1
        semantic_segmentation_binary_objects[mask_background | mask_drivable] = 0

        return semantic_segmentation_binary_objects
    ...
    def __getitem__(self, index):
        ...
        targets = {"boxes": self.boxes[index], "labels": self.labels[index], "name": self.images_name[index], "knowns": self.knowns[index], "unknowns": self.unknowns[index], "image_id": self.images_name[index], "img_size": image.shape}
        ...
        if self.semantics:
            ...
            targets["semantic_segmentation_binary_objects"] = self.semantic_segmentation_to_binary_objects(semantic_segmentation_2d)
