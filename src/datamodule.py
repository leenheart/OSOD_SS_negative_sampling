...
from datasets.predictions_dataset import PredictionsDataset
...
def coco_collate_fn(data):
    ...
    for d in data:
        ...
        targets[-1]["img_size"] = d[1]["img_size"]
...
def collate_fn(data):
    ...
    for d in data:
        ...
        if "semantic_segmentation" in d[1]:
            targets[-1]["semantic_segmentation"] = d[1]["semantic_segmentation"].to(float)
            targets[-1]["semantic_segmentation_binary_objects"] = d[1]["semantic_segmentation_binary_objects"].to(float)#.expand(3, -1, -1) 
            #targets[-1]["masks"] = d[1]["semantic_segmentation_binary_objects"].unsqueeze(0).unsqueeze(0)
...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(self, dataset_cfg, dataloader_cfg, model_classes_names, predictions_path, open_set=False, skip_train_load=False, collate_fn="list", transform=None):
        super().__init__()
        ...
        self.shuffle_in_training = False #dataloader_cfg.shuffle_in_training

        # Create datasets
        self.datasets = DatasetsWrapper(dataset_cfg, skip_train_load, model_classes_names, predictions_path, dataloader_cfg.num_workers, transform=transform)
...
def prediction_collate_fn(batch: list[tuple[dict[str, Union[Tensor, str]], dict[str, dict[str, Tensor]]]]) -> tuple[list[dict[str, Tensor]], dict[str, list[dict[str, Tensor]]]]:

    # Create empty dictionaries for collated tensors
    targets = []
    predictions = {}

    for model_name, first_prediction in batch[0][1].items():
        predictions[model_name] = {"filtered_predictions": [], "proposals": [],"scores": {}}
        for score_name in first_prediction["scores"].keys():
            predictions[model_name]["scores"][score_name] = []
    
    # Iterate over each batch element
    for target, models_predictions in batch:
        # Process the target
        targets.append(target)

        # Process the predictions
        for model_name, model_predictions in models_predictions.items():
            predictions[model_name]["filtered_predictions"].append(model_predictions["filtered_predictions"])
            predictions[model_name]["proposals"].append(model_predictions["proposals"])
            for score_name, score_values in model_predictions["scores"].items():
                predictions[model_name]["scores"][score_name].append(score_values)

    return targets, predictions

class PredictionsDataModule(pl.LightningDataModule):

    def __init__(self, data_dir, model_names, max_size, batch_size=32, num_workers=0, preload_predictions=True):
        super().__init__()
        self.data_dir = data_dir
        self.model_names = model_names
        self.batch_size = batch_size
        self.num_workers = num_workers
        self.dataset = PredictionsDataset(self.data_dir, self.model_names, max_size, preload_targets=True, preload_predictions=preload_predictions)

    def test_dataloader(self):
        return DataLoader(self.dataset, batch_size=self.batch_size, num_workers=0, collate_fn=prediction_collate_fn)
