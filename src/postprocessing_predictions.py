...
def calculate_sum_pixel_SS_in_boxes(boxes, semantic_segmentation):
    # Get the dimensions of the semantic segmentation image
    ...
    x_min, y_min, x_max, y_max = boxes[:, 0], boxes[:, 1], boxes[:, 2], boxes[:, 3] # 0, 0, 1344, 768
    
    # Create meshgrids for y and x coordinates
    x_indices, y_indices = torch.meshgrid(torch.arange(semantic_segmentation.shape[1], device=device), torch.arange(semantic_segmentation.shape[0], device=device)) # 1344, 768
    
    # Broadcast box coordinates to match image shape
    y_min = y_min.unsqueeze(1).unsqueeze(1)
    x_min = x_min.unsqueeze(1).unsqueeze(1)
    y_max = y_max.unsqueeze(1).unsqueeze(1)
    x_max = x_max.unsqueeze(1).unsqueeze(1)
    
    # Create masks for each box
    box_masks = (y_indices >= y_min) & (y_indices <= y_max) & (x_indices >= x_min) & (x_indices < x_max)


    nb_boxes = box_masks.size(0)

    sum_label_SS = torch.zeros(nb_boxes, device=device)
    for i in range(nb_boxes):
        box_mask = box_masks[i, : , :]
        sum_label_SS[i] = (semantic_segmentation.permute(1,0)[box_mask]).sum()

    #sum_label_SS /= height * width
...
def set_tags_opti(predictions, targets, batch_index, iou_threshold, considering_classes=True, tags_name="tags", add_centerness=False):

    pred_boxes = predictions[batch_index]["boxes"]
    pred_labels = predictions[batch_index]["labels"]
    ...
    targets[batch_index][tags_name] = torch.zeros(len(target_boxes), dtype=torch.bool, device=target_boxes.device)

    if num_preds == 0 or num_targets == 0:
        predictions[batch_index][tags_name] = torch.zeros(len(pred_boxes), dtype=torch.bool, device=target_boxes.device)
        predictions[batch_index]["IoU_" + tags_name] = torch.zeros(len(pred_boxes), device=target_boxes.device)
        if add_centerness:
            predictions[batch_index]["centerness"] = torch.zeros(len(pred_boxes), device=target_boxes.device)

        return

    # Calculate IoU between prediction and target boxes
    iou_all = box_iou(pred_boxes, target_boxes)

    # Mask for considering classes
    if considering_classes:
        class_mask = torch.eq(target_labels, pred_labels.unsqueeze(1))
        iou_all *= class_mask.float()

    # Find best match for each prediction
    iou_max, best_match_index = torch.max(iou_all, dim=1)

    # Filter predictions with IoU above threshold
    positive_mask = iou_max >= iou_threshold
    near_target_mask = iou_max > 0

    # Set tags for true positives
    predictions[batch_index][tags_name] = positive_mask
    predictions[batch_index]["IoU_" + tags_name] = iou_max * positive_mask

    # Set tags for matched target boxes
    targets[batch_index][tags_name][best_match_index[positive_mask]] = True

    # Calculate centerness if required
    if add_centerness:
        predictions[batch_index]["centerness"] = torch.zeros(len(pred_boxes), device=target_boxes.device)
        #target_boxes_matched = target_boxes[best_match_index[positive_mask]]
        #pred_boxes_matched = pred_boxes[positive_mask]
        #predictions[batch_index]["centerness"][positive_mask] = calculate_centerness_boxes(target_boxes_matched, pred_boxes_matched).squeeze(1)

        target_boxes_matched = target_boxes[best_match_index[near_target_mask]]
        pred_boxes_matched = pred_boxes[near_target_mask]
        predictions[batch_index]["centerness"][near_target_mask] = calculate_centerness_boxes(target_boxes_matched, pred_boxes_matched).squeeze(1)

def set_tags(predictions, targets, batch_index, iou_threshold, considering_classes=True, tags_name="tags", add_centerness=False):
    ...
    if add_centerness:
        predictions[batch_index]["centerness"] = torch.zeros(len(predictions[batch_index]["boxes"]), device=targets[batch_index]["boxes"].device)
    ...
    for prediction_index, pred_label in enumerate(predictions[batch_index]["labels"]):
        ...
        if add_centerness:
            target_box = targets[batch_index]["boxes"][best_match_index]
            pred_box = predictions[batch_index]["boxes"][prediction_index]

            predictions[batch_index]["centerness"][prediction_index] = calculate_centerness_box(target_box, pred_box)


def calculate_centerness_boxes(target_boxes: Tensor, pred_boxes: Tensor) -> Tensor:

    x_min_pred, y_min_pred, x_max_pred, y_max_pred = torch.chunk(pred_boxes, chunks=4, dim=1)
    x_pred_center = ((x_max_pred - x_min_pred) * 0.5) + x_min_pred
    y_pred_center = ((y_max_pred - y_min_pred) * 0.5) + y_min_pred

    x_min_target, y_min_target, x_max_target, y_max_target = torch.chunk(target_boxes, chunks=4, dim=1)

    l = x_pred_center - x_min_target
    r = x_max_target - x_pred_center
    t = y_pred_center - y_min_target
    b = y_max_target - y_pred_center

    #print(l[:10], r[:10], t[:10], b[:10])

    # Calculate centerness for each box in the batch
    centerness = torch.sqrt((torch.min(l, r) / torch.max(l, r)) * (torch.min(t, b) / torch.max(t, b)))
    centerness = torch.nan_to_num(centerness, nan=0.0)
...
def calculate_centerness_box(target_box, pred_box):
    x_min, y_min, x_max, y_max = pred_box
    x_pred_center = ((x_max - x_min) * 0.5) + x_min
    y_pred_center = ((y_max - y_min) * 0.5) + y_min

    x_min, y_min, x_max, y_max = target_box
    centerness = 0
    if x_min <= x_pred_center and x_max >= x_pred_center and y_min <= y_pred_center and y_max >= y_pred_center:
        l = x_pred_center - x_min
        r = x_max - x_pred_center
        t = y_pred_center - y_min
        b = y_max - y_pred_center

        centerness = math.sqrt((min(l, r) / max(l,r)) * (min(t, b)/max(t, b)))
    return centerness


def extract_predictions_with_mask(predictions, mask):

    new_predictions = {}
    for key, value in predictions.items():

        if not torch.is_tensor(value):
            print("something to do here on the key :", key)
            continue

        new_predictions[key] = value[mask]
        

    return new_predictions

def extract_batched_predictions_with_mask(predictions, masks):

    new_batched_predictions = []
    for idx, prediction in enumerate(predictions):
        new_predictions.append(extract_predictions_with_mask(prediction, mask[idx]))

    return new_batched_predictions

def extract_batched_predictions_with_nms(predictions, iou_threshold):

    new_batched_predictions = []
    for idx, prediction in enumerate(predictions):
        kept_boxes_from_nms = torchvision.ops.nms(prediction["boxes"], prediction['scores'], iou_threshold)  # NMS
        new_batched_predictions.append(extract_predictions_with_mask(prediction, kept_boxes_from_nms))
...
def get_only_targets(targets, on_background=False, on_known=False, on_unknown=False):
    ...
    for target in targets:
        ...
        for key, value in target.items():
            ...
            if value.numel() == 0 or key == "semantic_segmentation" or key == "semantic_segmentation_OBD" or key == "semantic_segmentation_binary_objects" or key == "masks":
